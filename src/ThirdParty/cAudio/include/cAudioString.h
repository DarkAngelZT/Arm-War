...
// Conversion is only required if using wide characters in cAudioString    
...
#ifdef CAUDIO_PLATFORM_WIN
        int buff_size = WideCharToMultiByte(CP_UTF8, 0, (LPWSTR)str.c_str(), (int)(str.size() < 1023 ? str.size() : 1023), buffer[id], 1023, 0, 0);
#else
    ...
    static cAudioString fromUTF8(const char* str)
    {
        ...
        MultiByteToWideChar(CP_UTF8, 0, str, str_len, (LPWSTR)s.c_str(), buf_size);
        return s;
    }
