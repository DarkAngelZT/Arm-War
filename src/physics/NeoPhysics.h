...
public:
	...
	void CompoundShapeAddChild(int shapeIndex,int childIndex, irr::core::vector3df localPos =
			irr::core::vector3df(0, 0, 0), irr::core::vector3df localRotation =
			irr::core::vector3df(0, 0, 0));
	//------rigid body-------
	int CreateRigidBody(int collisionShapeIndex,int sceneNodeIndex,float mass);
	//------joint-------
	int CreateHingeJoint(int rigidbody1,int rigidbody2,irr::core::vector3df pivot1, irr::core::vector3df pivot2, irr::core::vector3df axisIn1, irr::core::vector3df axisIn2);
	void HingeSetLimit(int hingeindex, float low,
            float high,
            float softness=0.9f,
            float _biasFactor=0.3f,
            float relaxationFactor=1.0f);
	void HingeEnableMotor(int hingeindex, bool enableMotor);
	void HingeEnablevoidAngularMotor (int hingeindex, bool enableMotor, float targetVelocity, float maxMotorImpulse);
	//=========================
...
private:
	...
	std::vector<std::shared_ptr<NeoMotionState>>m_motionStates;
