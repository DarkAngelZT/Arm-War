...
void BasicExplosionEffector::Play()
{
	...
	irr::scene::IParticleSystemSceneNode*ps_low =
			smgr->addParticleSystemSceneNode(false);
	//粒子系统,爆炸中心
	float totalParticleHighSpeed = m_params.fRadius * 30;
	irr::scene::IParticleSphereExplosionEmitter* emitter_high_speed =
			irr::scene::CreateExplosionSphereEmitter(vector3df(0,0,0), //中心位置(相对坐标)
					m_params.fRadius, //emitter半径
					vector3df(0, m_params.fRadius / 500.f, 0), //初始方向
					totalParticleHighSpeed*5,
					totalParticleHighSpeed*10, //每秒最少、最多发射粒子数
	...
	ps_high->setMaterialType(video::EMT_TRANSPARENT_ALPHA_CHANNEL);
